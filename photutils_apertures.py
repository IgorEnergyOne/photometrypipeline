...
class PillBoxAperture(PillBoxMaskMixin, PixelAperture):
    ...
    @property
    def area(self):
        return (self.w - self.a - self.b) * self.h + np.pi * self.a * self.b

    @staticmethod
    def create_pill_aperture(x_center, y_center, width, height, angle):
        """
        Create a pill-shaped (stadium) aperture as a polygon.

        Parameters:
        -----------
        x_center, y_center : float
            Center coordinates of the aperture
        width : float
            Total width of the pill (including semicircular ends)
        height : float
            Height of the pill (diameter of semicircular ends)
        angle : float
        ...
        polygon_points : numpy array
            Array of (x, y) points defining the pill shape
        """
        ...
        straight_length = max(0, width - height)
        half_straight = straight_length / 2

        # Create points for the pill shape (before rotation)
        points = []

        # Number of points for each semicircle
        n_points = 25

        # Start from top-right, go counterclockwise
        ...
        for i in range(n_points + 1):
            theta = np.pi / 2 + np.pi * i / n_points  # From π/2 to 3π/2
            x = -half_straight + radius * np.cos(theta)
        ...
        for i in range(n_points + 1):
            theta = -np.pi / 2 + np.pi * i / n_points  # From -π/2 to π/2
            x = half_straight + radius * np.cos(theta)
            y = radius * np.sin(theta)
            points.append([x, y])
        ...
        rotated_points[:, 0] += x_center
        rotated_points[:, 1] += y_center
    ...
    def _to_patch(self, origin=(0, 0), indices=None, **kwargs):
        ...
        """
        import matplotlib.patches as mpatches
        # Create pill-shaped aperture
        xy_positions, patch_kwargs = self._define_patch_params(origin=origin, **kwargs)
        # theta_deg = (self.theta * 180. / np.pi)
        theta_deg = self.theta.value
        pill_aperture = PillBoxAperture.create_pill_aperture(xy_positions[0][0],
                                             xy_positions[0][1],
                                             self.w, self.h, theta_deg)

        # Add the aperture to the plot
        aperture_patch = Polygon(pill_aperture, fill=False, edgecolor='red',
                                     linewidth=0.5, linestyle='-', alpha=1)
        return aperture_patch
