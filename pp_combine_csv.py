...
def combine_csv_files(path_core_dir: str,
    ...
    Args:
        path_core_dir: Base directory path (only used if dir_name_pattern is a string pattern)
        dir_name_pattern: Either a string pattern to match directories or a list of directory paths
        file_name_pattern: Pattern to match CSV files within the directories
        out_file_path: Path to save the combined CSV
        append: If True, append to existing file instead of overwriting
    """
    if isinstance(dir_name_pattern, list):
        # If a list of directories is provided, use them directly
        dirs = [Path(d) for d in dir_name_pattern]
    else:
        # Otherwise, treat it as a pattern and find matching directories
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='combine results of photometry from different directories into one file')
    ...
    parser.add_argument('-dirs_pattern',
                        help='name pattern for directories or a comma-separated list of directory paths',
                        type=lambda x: [d.strip() for d in x.split(',')] if ',' in x else x,
                        default="*")
    ...
    dir_name_pattern = args.dirs_pattern
